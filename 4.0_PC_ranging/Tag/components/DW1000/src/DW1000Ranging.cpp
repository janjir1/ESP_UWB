			...
			if(_type == ANCHOR) {
				...
				if(messageType == POLL) {
					...
					for(uint8_t i = 0; i < numberDevices; i++) {
						...
						Serial.print(shortAddress[0]);
						Serial.print(shortAddress[1]);
						Serial.println(SHORT_MAC_LEN+2+i*RANGE_LEN);
						visualizeDatas(data);
...
void DW1000RangingClass::printKnownInfo() { _printInfo = true; }
